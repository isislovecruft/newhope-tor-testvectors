...
void helprec(unsigned char *r, const poly *v, const unsigned char *seed, unsigned char nonce)
{
  ...
  for(i=0;i<PARAM_N/4;i++)
    r[i] = c.coeffs[4*i] | (c.coeffs[4*i+1] << 2) | (c.coeffs[4*i+2] << 4) | (c.coeffs[4*i+3] << 6);
}


void rec(unsigned char *key, const poly *v, const unsigned char *r)
{
  int i;
  int32_t tmp[4];
  poly c;
